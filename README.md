...
without this, everything resets every time you redeploy):
   - You only need **one volume** for this app (an earlier version of these instructions
     said two — that's been simplified, see note below).
   - On the Railway canvas, right-click empty space (or use the stacked-layers icon in the
     left toolbar) → **Create volume**. When it asks for a mount path, type `/app/data` and
     confirm.
   - Drag a connector from the new volume to your service if it doesn't attach on its own.
   - That's it — both the database and every uploaded photo live under `/app/data` now, so
     this single volume covers everything.
   - *(Why the change: uploaded photos used to be stored inside `public/uploads`, which
     would have needed its own separate volume — and mounting a volume over a folder that
     also has app code in it can hide that code from the running app. The app now stores
     uploads under `data/uploads` instead, right alongside the database, and copies the
     real event photos in automatically the first time it starts on a fresh volume — so one
     volume mounted at `/app/data` is genuinely enough.)*
